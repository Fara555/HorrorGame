...
public class AiLaughingState : AiState
{
    ...
    private IEnumerator Laugh(AiAgent agent)
    {
        ...
        if (agent.attackSensor.IsInSight(agent.playerTransform.gameObject) || agent.backSensor.IsInSight(agent.playerTransform.gameObject)) agent.stateMachine.ChangeState(AiStateId.Attack); // Check if player still close enough for another attack
        else if (agent.sensor.IsInSight(agent.playerTransform.gameObject)) agent.stateMachine.ChangeState(AiStateId.ChasePlayer); // Then check if player close enough to immediately start chasing 
